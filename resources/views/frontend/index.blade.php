...
<meta name="facebook-domain-verification" content="0zzjif0tgu2pg8uypm6v1cx22tfc2f" />
  ...
  body{
    background:url({{url('/dashboard')}}) center,
    url({{url('public/frontend/images/my-loader.svg')}}) center center no-repeat ;/* this works once/untill image has been loaded */
...
<div class="dropdown">
  ...
  <div id="card">
    ...
    <?php echo $retVal = (Session::has('area_name')) ? "value='".Session::get('area_name')."'" :"value=''  " ;?>
...
<?php 
        $open_q_tab = false;
        $current_quest = false;
        if(Session::has('current_q_id')){
          if(Session::get('current_q_id') == $questions->q_id ){
            $open_q_tab = true;
          }
    ...
    <div class="accordion-item">
      <h2 class="accordion-header" id="panelsStayOpen-headingOne">
        <button class="accordion-button    @if(!$open_q_tab)  collapsed @endif  " type="button" data-bs-toggle="collapse" data-bs-target="#question_no_{{ $questions->q_id }}" @if($open_q_tab) aria-expanded="true" @else aria-expanded="false" @endif  aria-controls="question_no_{{ $questions->q_id }}">
      {{ $questions->question }}
        </button>
      </h2>
      <div id="question_no_{{ $questions->q_id }}" class="accordion-collapse collapse @if($open_q_tab) show @endif" aria-labelledby="panelsStayOpen-headingOne">
        ...
        <div  class="row">
          ...
          <input type="radio" value="1" class="btn-check" name="options" id="option_{{ $questions->q_id }}_1" autocomplete="off"  />
            ...
            if($totalQuesVotes1 > 0 && $totalVotes1 > 0){
                $per1 = number_format(($totalVotes1/$totalQuesVotes1)*100,0);
                if($totalVotes1 > 1){$vote_text1 = 'Votes';}
            }
            ?>
            <label class="btn btn-secondary {{$btn_val1}} options qoptions_{{ $questions->q_id }}" for="option1" id="{{ $questions->q_id }}_1" ids="{{ $questions->q_id }}_1">{{ $questions->opt1 }}</label>
                <div class=" p_voutes"> <span id="t_votes_{{ $questions->q_id }}_1">{{$totalVotes1}}</span> <span id="text_votes_{{ $questions->q_id }}_1">{{$vote_text1}}</span> (<span id="t_per_{{ $questions->q_id }}_1">{{$per1}}</span><span>%</span>)</div>
            @endif
    ...
    @if($questions->opt2 !="")
        ...
        $per2 = 0;
          ...
          if($totalQuesVotes2 > 0 && $totalVotes2 > 0){
              ...
              $per2 = number_format(($totalVotes2/$totalQuesVotes2)*100,0);
              if($totalVotes2 > 1){$vote_text2 = 'Votes';}
          }
          ?>
          ...
          <div class=" p_voutes"> <span id="t_votes_{{ $questions->q_id }}_2">{{$totalVotes2}}</span> <span id="text_votes_{{ $questions->q_id }}_2">{{$vote_text2}}</span> (<span id="t_per_{{ $questions->q_id }}_2">{{$per2}}</span><span>%</span>)</div>
      @endif
    </div>
    <div class="col-3 text-center">
    @if($questions->opt3 !="")
      ...
      <?php
         ...
         $per3 = 0;
          ...
          if($totalQuesVotes3 > 0 && $totalVotes3 > 0){
              ...
              $per3 = number_format(($totalVotes3/$totalQuesVotes3)*100,0);
              if($totalVotes3 > 1){$vote_text3 = 'Votes';}
          }
          ?>
      <label class="btn btn-secondary {{$btn_val3}} options options qoptions_{{ $questions->q_id }}" for="option1" id="{{ $questions->q_id }}_3" ids="{{ $questions->q_id }}_3">{{ $questions->opt3 }}</label>
          <div class=" p_voutes"> <span id="t_votes_{{ $questions->q_id }}_3">{{$totalVotes3}}</span> <span id="text_votes_{{ $questions->q_id }}_3">{{$vote_text3}}</span> (<span id="t_per_{{ $questions->q_id }}_3">{{$per3}}</span><span>%</span>)</div>
      @endif
</div>
    <div class="col-3 text-center">
    @if($questions->opt4 !="")
    ...
    <?php
          ...
          if($totalQuesVotes4 > 0 && $totalVotes4 > 0){
              ...
              $per4 = number_format(($totalVotes4/$totalQuesVotes4)*100,0);
              if($totalVotes4 > 1){$vote_text4 = 'Votes';}
          }
          ?>
    <label class="btn btn-secondary {{$btn_val4}} options options qoptions_{{ $questions->q_id }}" for="option4" id="{{ $questions->q_id }}_4" ids="{{ $questions->q_id }}_4">{{ $questions->opt4 }}</label>
          <div class=" p_voutes"> <span id="t_votes_{{ $questions->q_id }}_4">{{$totalVotes4}}</span> <span id="text_votes_{{ $questions->q_id }}_4">{{$vote_text4}}</span> (<span id="t_per_{{ $questions->q_id }}_4">{{$per4}}</span><span>%</span>)</div>
      @endif
    </div>
    </div>
    ...
    <div class="cont" id="question_{{ $questions->q_id }}">



          
          <!--<div style="padding:none;" class="comment-wrapper9 container container_{{ $questions->q_id }} m-3">-->
          <div style="padding:0px; padding-right: 10px;margin-left:7px;" class="comment-wrapper9 container container_{{ $questions->q_id }}">
...
// echo $u_id.' => '.$comments->count();
       ?>
                  ...
                  @if($comments)
                          ...
                          <div style="padding:4px;" class="user-comment single-item id_{{ $questions->q_id }}" data-id='{{ $questions->q_id }}'>
                 ...
                 <div style="margin-top: 5px;    margin-bottom: -10px;">{!! $comments->appends(['id' => $questions->q_id])->links('frontend/pagination_html', ['data'=>$comments]) !!}</div>
                @endif
                </div>
...
@endforeach()
          <?php
                        ...
                        $raw_speed  = round($kb / $deltat, 3);
                        $mpbs = $raw_speed/1024*1024;
                        //echo "Your Internete Speed is ". $raw_speed."Mbps";
                        */
                        ?>
  </div>
...
$(document).ready(function() {
  ...
  const successCallback = (position) => {
    ...
    //Get current location longitude and latitude, if user allow then set session otherwise not.
    //var current_address = getAddressFromLatLng(latitude, longitude);
    ...
    geocoder.geocode({ 'location': latlng }, function(results, status) {
      if (status === google.maps.GeocoderStatus.OK) {
        if (results[0]) {
          var address = results[0].formatted_address;
          //console.log("Location address: " + address);
          //write ajax here start
          @if(!Session::has('area_name'))
            if(latitude !='' && longitude != '' ){
              $.ajax({
                          type: 'post',
                          url: "{{url('setUserDefaultLocation')}}",
                          ...
                          success: function(data) {
                            ...
                            //console.log('success');
                            //console.log(data);
                            if(data.status == true && data.address != ''){
                              location.reload();
                            }
...
// });
  ...
  if (location.trim() !== "") {
    ...
    // console.log('Add location border if input has a value during load time');
    ...
    $(qoptionsclass).each(function() {
          ...
          $.ajax({
              ...
              success:function(data){
                  ...
                  var vote_no = data.vote_no;
                  var u_id = data.u_id;
                  var t_votes = parseInt(data.t_votes);
                  var per = parseInt(data.per);

                  var vote_id = '#t_votes_'+q_id+'_'+vote_no;
                  var vote_text_id = '#text_votes_'+q_id+'_'+vote_no;
                  ...
                  $(vote_text_id).html(vote_text);
                  $(vote_per).html(per.toFixed(0));
              },
              error:function(data){
                  // console.log('error');
                  // console.log(data.responseText);
...
<script>
    $(document).ready(function(){
        $(document).on('click', '.pagination a', function(e){
                ...
                var id_data = $(this).attr('href').split('id=')[1];
                var id = id_data.split('&')[0];
                //console.log(id_data);
                //console.log(id);
                //console.log(page);
                fetch_data(page, id);
        ...
        function fetch_data(page, id){
            ...
            $.ajax({
                url:base_url+"/pagination/fetch_data?id="+id+"&page="+page,
                success:function(data){
